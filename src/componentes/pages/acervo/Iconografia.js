...
export const Iconografia = () => {
  ...
  const getTemas = async () => {
    const url = "https://backend-prueba-apel.onrender.com/api/iconografia/listar-temas";
  ...
  const handleTemaClick = (tema) => {
    navigate(`/admin/iconografia/tema/${tema}`);
  };
          ...
          {temas.map((tema, index) => (
            ...
            >
              <article>
                <img
                  id='img_temas'
                  src={`https://backend-prueba-apel.onrender.com/imagenes/iconografia/${tema.nombreImagen}`}
                  alt={`${tema.tema} foto`}
                />
