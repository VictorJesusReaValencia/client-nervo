...
export const RegDocumentacion = () => {
    ...
    const guardar_foto = async (e) => {
            ...
            const { datos } = await Api("http://localhost:3900/api/documentacion/registrar", "POST", nueva_foto);
            ...
            if (datos.status === "success") {
                ...
                const subida2 = await Api(`http://localhost:3900/api/documentacion/registrar-imagen/${datos.publicacionGuardada._id}`, "POST", formData, true);
                ...
                const pdfSubida2 = await Api(`http://localhost:3900/api/documentacion/registrar-pdfs/${datos.publicacionGuardada._id}`, "POST", pdfFormData, true);
    ...
    /*const handleAutoComplete = async (field, promptId) => {
        ...
        if (fileInput.files.length > 0) {
            ...
            const { datos } = await Api(`http://localhost:3900/api/documentacion/gpt/image-text/${promptId}`, "POST", formData, true);
